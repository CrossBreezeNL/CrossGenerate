...
public class XGenerateStarter extends GenerationObserverSource {
	...
	public XGenerateStarter(String[] args) {			
		...
		//Set trail date
		Calendar myCalendar = new GregorianCalendar(2020, 6, 30);
		trailEndDate = myCalendar.getTime();
	...
	private void startGenerator(XGenAppConfig appConfig, ArrayList<ModelTemplateConfigCombination> modelTemplateConfigCombinations, boolean debugMode) {
		try {
			...
			logger.warning("This is a trail version of CrossGenerate that expires on " + trailEndDate);		
			if (!this.trailIsValid()) {
				return;
			}
			GeneratorStub generator = new Generator();
	...
	private boolean trailIsValid() throws GeneratorException {
		try {
			Date compileDate = new Date(this.getClass().getResource("XGenerateStarter.class").openConnection().getLastModified());
			Date currentDate = new Date();
			
			//If current date is smaller then compile date, the system date is probably set back in time
			if (compileDate.after(currentDate)) {
				logger.warning("Trail license is expired");
				return false;
			}
			
			//if current date is larger then 30-6-2020
			if (currentDate.after(this.trailEndDate)) {
				logger.warning("Trail license is expired");
				return false;
			}
			return true;
		} catch (IOException e) {
			throw new GeneratorException("Unable to determine if trail license is still valid");
		}
